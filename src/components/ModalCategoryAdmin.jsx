...
export default function ModalCategoryAdmin({
  isVisible,
  closeModal,
  purpose,
  category,
  ...
  async function updateHandler() {
    if (savedDataToken) {
      const res = await axiosInstance.patch('/categories/' + category.id, category, {
        headers: {
          Authorization: `Bearer ${savedDataToken}`
        }
      });
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black bg-opacity-20">
      {purpose === 'create' ? (
        ...
        <div className="flex flex-col h-2/6 w-2/6 rounded-2xl bg-red-400 border-4 border-black shadow-[10px_10px]">
          ...
          <hr className="border border-black" />
          <div className="flex grow flex-col items-center justify-center gap-5 bg-white rounded-b-xl">
            <div className="font-bold">Are you sure want to delete this category ?</div>
            <div className="flex gap-2">
              <button
