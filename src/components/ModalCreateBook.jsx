...
export default function ModalCreateBook({ isVisible, closeModal, setChange }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      formData.append('tittle', formik.values.tittle);
      formData.append('synopsis', formik.values.synopsis);
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex flex-col items-center bg-black justify-center bg-opacity-30 py-12">
      <div
        className={`flex flex-col h-4/6 w-1/2 rounded-2xl bg-green-400 border-4 border-black shadow-[15px_15px]`}>
        ...
        <div className="flex grow items-center justify-evenly gap-2 bg-white rounded-b-xl">
          {
            <div
              className="w-1/3 h-2/3 flex items-center justify-center cursor-pointer rounded-md border-2 border-black shadow-[8px_8px]"
              onClick={() => inputFileRef.current.click()}>
              {image_url ? (
                <img src={image_url} alt="" className="w-full h-full object-cover rounded-md" />
              ) : bookCover ? (
                <img
          ...
          <div className="basis-1/2 flex flex-col gap-2">
            <div className="w-5/6 flex flex-col">
              <label htmlFor="tittle" className="font-bold">
                Tittle
              </label>
              <input
                id="tittle"
                type="text"
                placeholder="Tittle"
                defaultValue={formik.values.tittle}
                onChange={(e) => {
            ...
            <div className="w-5/6 flex flex-col">
              ...
              <textarea
                ...
                cols="30"
                rows="3"
                defaultValue={formik.values.synopsis}
                onChange={(e) => formik.setFieldValue('synopsis', e.target.value)}
